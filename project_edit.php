<?php
	...
	if ($edit_id) {
		...
		$get_edit_data_sql = "SELECT * FROM our_projects_infos WHERE id = '$edit_id'";
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $project_detail_en = $_POST["project_detail_en"];
        $project_detail_bn = $_POST["project_detail_bn"];

		$edit_id = $_POST['edit_id'];
		
		// Edit existing record
		$edit_sql = "UPDATE our_projects_infos SET 
			title_en='$title_en', 
			title_bn='$title_bn', 
			project_detail_en='$project_detail_en', 
			project_detail_bn='$project_detail_bn'
			WHERE id=$edit_id";
		...
		if ($conn->query($edit_sql) === TRUE) {
			if (!empty($_FILES['img']['tmp_name'])) {
                ...
                foreach ($uploadedFiles['name'] as $key => $value) {
                    ...
                    $insert_file_sql = "INSERT INTO our_projects_infos_file (galleries_id, img) 
...
<div class="container-fluid">
    <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
			...
			<?php } ?>
			<form action="project_edit.php" method="post" enctype="multipart/form-data" style="border: 1px solid #ddd;padding: 20px;border-radius: 4px;background: aliceblue">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<label for="project_detail_en">Details (English):</label>
						<textarea class="form-control" id="project_detail_en" name="project_detail_en" required><?php echo $get_data_col["project_detail_en"];?></textarea><br>						
						
						<label for="project_detail_bn">Details (Bangla):</label>
						<textarea class="form-control" id="project_detail_bn" name="project_detail_bn" required><?php echo $get_data_col["project_detail_bn"];?></textarea><br>
					...
					<div class="col-md-12">
						...
						<div id="imageContainer">
							...
							<?php
								$image_data_sql = "SELECT * FROM our_projects_infos_file WHERE project_id = '$edit_id'";
					...
					<div class="col-md-6">
						<br>
						<a class="btn btn-danger btn-block" href="project.php">Back</a>
					</div>
...
<script>
    ...
    if (confirm("Are you sure you want to delete this image?")) {
        ...
        xhttp.open("GET", "project_image_delete.php?id=" + imageId, true);
        xhttp.send();
		
		setTimeout(function() {
			window.location.href = "project_edit.php?id=<?php echo $edit_id; ?>";
		}, 100);
